export function buildSystemPrompt(
  ...
  const pageList =
    pages.length > 0
      ? pages.map((p) => `- [${p.id}] **${p.title}**: ${p.summary}`).join("\n")
...
Each entry is formatted as "[page_id] Title: summary". Use the page_id directly when calling the update_wiki_page tool.
